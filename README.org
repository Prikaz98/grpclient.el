...
Call ~grpclient-completion-complete~ (~C-c C-c~ in ~grpclient-mode~) or ~M-x~.
...
|--------------------------------------+-------------+-------------------------------|
| ~grpclient-completion-complete~      | ~C-c C-c~   | Insert gRPC request at point  |
| ~grpclient-completion-refresh-cache~ | ~M-x <cmd>~ | Force-refetch reflection data |
...
| C-c C-c | grpclient-completion-complete       |
